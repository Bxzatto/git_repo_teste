    ...
    <b-row class="justify-content-center">
          ...
          <b-input-group class="mt-4">
            ...
            <b-input-group-append>
              <b-button variant="light" v-on:click="test(search)">
                <b-icon-search style="color: black; font-weight: bold;"></b-icon-search>
              </b-button>
            </b-input-group-append>
          </b-input-group>
        </div>
      ...
      <b-col cols="12">
        <b-row v-for="item in results" :key="item.id">
          <b-col cols="12" class="d-flex justify-content-center align-items-center">
            <CardComponent :description="item.description" :name="item.full_name" :img="item.owner.avatar_url" />
          </b-col>
        </b-row>
      </b-col>
    </b-row>
...
export default {
  ...
  methods: {
    test: function (search) {
      fetch(`https://api.github.com/search/repositories?page=2&per_page=5&q=${search}&sort=stars&order=desc`)
        .then(async response => {
